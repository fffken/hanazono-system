...
def _generate_battery_soc_chart(self, data, date_str):
    ...
    try:
        ...
        if not soc_values or len(soc_values) < 2:
            self.logger.warning(
                f"グラフ生成: SOCデータが不足しています（点数: {len(soc_values)}）")
            return None
    ...
    def send_daily_report(self, date=None):
        ...
        try:
            ...
            body_text = body_text.replace(
                'LVYUANソーラー蓄電システム日次レポート', 'HANAZONOシステム 日次レポート')
    ...
    def _generate_text_report(self, date_str, battery_data, season_info, recommended_settings, weather_data):
        ...
        # パラメータID
        charge_current_id = self.settings.get(
            'inverter_parameters', {}).get('charge_current_id', '07')
        charge_time_id = self.settings.get(
            'inverter_parameters', {}).get('charge_time_id', '10')
        soc_setting_id = self.settings.get(
            'inverter_parameters', {}).get('soc_setting_id', '62')
        ...
        soc = recommended_settings.get(
            'output_soc', recommended_settings.get('cutoff_soc', 'N/A'))
    ...
    def _send_email(self, subject, body_text, body_html=None, to_email=None, attachments=None):
        ...
        try:
            ...
            msg["Subject"] = subject
            msg["From"] = self.settings.get("email", {}).get(
    ...
    def send_alert(self, to_email=None, subject=None, body=None):
        ...
        """
        if to_email is None:
            to_email = self.settings.get('admin_email', self.settings.get(
                'email', {}).get('recipients', [''])[0])
