...
class ReportGenerator:
    ...
    def __init__(self, data_dir=None):
        if data_dir is None:
            self.data_dir = os.path.join(os.path.expanduser('~'), 'lvyuan_solar_control', 'data')
        else:
    ...
    def _get_value_at_time(self, timestamps, values, target_hour):
        ...
        for t, v in zip(timestamps, values):
            dt = datetime.datetime.fromtimestamp(t) if isinstance(t, (int, float)) else t
        ...
        if best_value is None:
            for t, v in zip(timestamps, values):
                dt = datetime.datetime.fromtimestamp(t) if isinstance(t, (int, float)) else t
                diff = abs((dt.hour - target_hour) * 3600 + dt.minute * 60 + dt.second)
                if diff < best_diff:
                    best_diff = diff
                    best_value = v
        return best_value
    ...
    def generate_daily_report(self, date_str=None):
        ...
        report = {'date': date_str, 'formatted_date': formatted_date, 'data_points': len(data), 'first_timestamp': first_entry.get('datetime', ''), 'last_timestamp': last_entry.get('datetime', ''), 'soc_morning': morning_soc, 'soc_night': night_soc, 'soc_min': min(soc_values) if soc_values else 0, 'soc_max': max(soc_values) if soc_values else 0, 'voltage_avg': sum(voltage_values) / len(voltage_values) if voltage_values else 0, 'current_avg': sum(current_values) / len(current_values) if current_values else 0}
        self._generate_daily_charts(date_str, formatted_date, dt_timestamps, soc_values, voltage_values, current_values)
        report_filename = os.path.join(self.reports_dir, f'daily_report_{date_str}.json')
    ...
    def _generate_daily_charts(self, date_str, formatted_date, timestamps, soc_values, voltage_values, current_values):
        ...
        plt.plot(timestamps, soc_values, 'b-', marker='o', label='バッテリーSOC (%)')
        ...
        fig.tight_layout()
        plt.gcf().autofmt_xdate()
        voltage_current_chart_path = os.path.join(self.charts_dir, f'voltage_current_{date_str}.png')
    ...
    def generate_weekly_report(self, end_date=None):
        ...
        if not daily_reports:
            ...
            return None
        valid_morning_soc = [r['soc_morning'] for r in daily_reports if r['soc_morning'] is not None]
        valid_night_soc = [r['soc_night'] for r in daily_reports if r['soc_night'] is not None]
        soc_morning_avg = sum(valid_morning_soc) / len(valid_morning_soc) if valid_morning_soc else 0
        soc_night_avg = sum(valid_night_soc) / len(valid_night_soc) if valid_night_soc else 0
        weekly_report = {'period': f'{dates[-1]} - {dates[0]}', 'formatted_period': f'{dates[-1][:4]}年{dates[-1][4:6]}月{dates[-1][6:8]}日～{dates[0][:4]}年{dates[0][4:6]}月{dates[0][6:8]}日', 'days_count': len(daily_reports), 'soc_morning_avg': soc_morning_avg, 'soc_night_avg': soc_night_avg, 'daily_reports': daily_reports}
        self._generate_weekly_charts(weekly_report)
        timestamp = time.strftime('%Y%m%d')
        report_filename = os.path.join(self.reports_dir, f'weekly_report_{timestamp}.json')
        with open(report_filename, 'w') as f:
    ...
    def _generate_weekly_charts(self, weekly_report):
        ...
        morning_soc = [r['soc_morning'] for r in weekly_report['daily_reports']]
        night_soc = [r['soc_night'] for r in weekly_report['daily_reports']]
        dates.reverse()
        morning_soc.reverse()
        night_soc.reverse()
        plt.figure(figsize=(12, 6))
        plt.bar(dates, morning_soc, width=0.4, align='edge', color='skyblue', label='朝7時 SOC')
        plt.bar([d for d in dates], night_soc, width=-0.4, align='edge', color='navy', label='夜23時 SOC')
        ...
        plt.ylim(0, 100)
        timestamp = time.strftime('%Y%m%d')
        weekly_chart_path = os.path.join(self.charts_dir, f'weekly_soc_{timestamp}.png')
    ...
    def generate_monthly_report(self, end_date=None):
        ...
        soc_morning_avg = sum(valid_morning_soc) / len(valid_morning_soc) if valid_morning_soc else 0
        soc_night_avg = sum(valid_night_soc) / len(valid_night_soc) if valid_night_soc else 0
        monthly_report = {'period': f'{dates[-1]} - {dates[0]}', 'formatted_period': f'{dates[-1][:4]}年{dates[-1][4:6]}月{dates[-1][6:8]}日～{dates[0][:4]}年{dates[0][4:6]}月{dates[0][6:8]}日', 'days_count': len(daily_reports), 'soc_morning_avg': soc_morning_avg, 'soc_night_avg': soc_night_avg, 'daily_reports': daily_reports}
        self._generate_monthly_charts(monthly_report)
        timestamp = time.strftime('%Y%m%d')
        report_filename = os.path.join(self.reports_dir, f'monthly_report_{timestamp}.json')
        with open(report_filename, 'w') as f:
    ...
    def _generate_monthly_charts(self, monthly_report):
        ...
        morning_soc = [r['soc_morning'] for r in monthly_report['daily_reports']]
        ...
        plt.tight_layout()
        timestamp = time.strftime('%Y%m%d')
        monthly_chart_path = os.path.join(self.charts_dir, f'monthly_soc_{timestamp}.png')
