...
def fix_recommendation_icon():
    """推奨変更アイコン修正"""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    print(f"🎨 推奨変更アイコン修正開始 {timestamp}")
    
    fixed_file = f"abc_integration_icon_fixed_{timestamp}.py"
...
ID 07: {base['ID07']}A (基本)    ID 10: {base['ID10']}分 (基本)    ID 62: {base['ID62']}% (基本)"""
    ...
    def send_icon_fixed_email(self, weather_data, battery_info, recommendation_data):
        """アイコン修正版メール送信"""
        try:
            ...
            body = f"""HANAZONOシステム A・B・C統合アイコン修正版 {datetime.datetime.now().strftime('%Y年%m月%d日 (%H時)')}
...
✅ C. SettingRecommender統合: アイコン修正対応推奨設定
...
--- HANAZONOシステム A・B・C統合アイコン修正版 ---"""
            ...
            print("✅ アイコン修正版メール送信成功")
            print(f"📧 件名: {subject}")
            print(f"🎨 推奨アイコン: {recommendation_data['recommendation_icon']} 推奨変更")
            return True
            
        except Exception as e:
            print(f"❌ アイコン修正版メール送信エラー: {e}")
            return False
            
    def run_icon_fixed_test(self):
        """アイコン修正版テスト実行"""
        print("🎯 アイコン修正版テスト開始")
        print("=" * 70)
        ...
        result = self.send_icon_fixed_email(weather_data, battery_info, recommendation_data)
        
        if result:
            print("\\n🎨 アイコン修正完成！メール受信確認をお願いします")
            print(f"📧 表示: {recommendation_data['recommendation_icon']} 推奨変更")
        
        return result

if __name__ == "__main__":
    icon_fixed_system = ABCIntegrationIconFixed()
    icon_fixed_system.run_icon_fixed_test()
'''
    
    with open(fixed_file, 'w', encoding='utf-8') as f:
        f.write(fixed_code)
        
    print(f"✅ アイコン修正版作成: {fixed_file}")
    print(f"\n🧪 アイコン修正版テスト実行:")
    print(f"python3 {fixed_file}")
    
    print(f"\n🎨 修正内容:")
    print(f"❌ 変更前: 🎯 推奨変更")
    print(f"✅ 変更後: 🟠 推奨変更（晴れ時）")
    print(f"✅ 変更後: 🔵 推奨変更（雨時）")
    print(f"✅ 変更後: 🟣 推奨変更（曇天時）")
    print(f"✅ 変更後: 🌻 推奨変更（変更不要時）")
    
    return fixed_file

if __name__ == "__main__":
    fix_recommendation_icon()
