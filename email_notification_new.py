...
class EmailNotifier:
    ...
    def find_latest_data_file(self, target_date_str=None, data_dir='data'):
        ...
        target_file = os.path.join(
            data_dir, f"lvyuan_data_{target_date_str}.csv")
    ...
    def send_daily_report(self):
        ...
        try:
            ...
            # 前日または最新のデータファイルを検索
            data_file, actual_date_str = self.find_latest_data_file(
                yesterday_str)
            ...
            if use_fallback:
                print(
                    f"指定された日付 {yesterday_str} のデータがないため、日付 {actual_date_str} のデータを使用します")
