...
def setup_logger():
    ...
    file_format = logging.Formatter('%(asctimes - %(levelname)s - %(message)s')
...
def log_settings(weather_data, season, settings_type, settings):
    ...
    引数:
        ...
        settings: 適用する設定値
    """
    season_names = {'winter': '冬季', 'spring_fall': '春秋季', 'summer': '夏季'}
    type_names = {'typeA': 'タイプA（控えめ設定）', 'typeB': 'タイプB（標準設定）'}
    logger.info('----- 設定推奨 -----')
    logger.info(f"天気: 今日={weather_data['today']['weather']}, 明日={weather_data['tomorrow']['weather']}")
