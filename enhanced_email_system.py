...
class EnhancedEmailSystem:
    ...
    def __init__(self, settings_manager, logger=None):
        ...
        self.weather_emojis = {'晴れ': '☀️', '曇り': '☁️', '雨': '🌧️', '雪': '❄️', '不明': '🌫️'}
        self.targets = {'solar_generation': 25.0, 'battery_efficiency': 0.85, 'grid_independence': 0.7}
        self.cost_rates = {'grid_purchase': 28.5, 'feed_in_tariff': 17.0}
        self.annual_savings_target = 200000
    ...
    def _calculate_daily_achievements(self, solar_data):
        ...
        achievements = {'solar': {'rate': solar_rate, 'message': f'{solar_generation:.1f}kWh 発電', 'emoji': self._get_achievement_emoji(solar_rate / 100), 'grade': self._get_grade(solar_rate / 100)}, 'battery': {'rate': battery_rate, 'message': f'{battery_efficiency:.1%} 効率', 'emoji': self._get_achievement_emoji(battery_efficiency), 'grade': self._get_grade(battery_efficiency)}}
        return achievements
    ...
    def _calculate_cost_savings(self, solar_data):
        ...
        grid_rate = self.cost_rates['grid_purchase']
        ...
        cost_analysis = {'daily_savings': daily_savings, 'monthly_projection': monthly_projection, 'yearly_projection': yearly_projection, 'target_achievement': target_achievement, 'emoji': '💰' if target_achievement >= 80 else '📊'}
    ...
    def _generate_recommendations(self, weather_data, achievements):
        ...
        if weather_data:
            ...
            if '晴れ' in today_weather:
                recommendations.append({'parameter': 'charge_current', 'value': 15, 'unit': 'A', 'reason': '☀️ 晴天予報のため充電電流を増加', 'param_id': '07', 'priority': 'high', 'emoji': '⚡', 'expected_savings': 180})
            elif '雨' in today_weather:
                recommendations.append({'parameter': 'output_soc', 'value': 70, 'unit': '%', 'reason': '🌧️ 雨天予報のため出力SOCを上げて節電', 'param_id': '62', 'priority': 'medium', 'emoji': '🔋', 'expected_savings': 120})
        if achievements.get('solar', {}).get('rate', 0) < 80:
            recommendations.append({'parameter': 'boost_charge_time', 'value': 60, 'unit': '分', 'reason': '📊 発電量が目標を下回っているため充電時間を延長', 'param_id': '10', 'priority': 'medium', 'emoji': '⏰', 'expected_savings': 150})
    ...
    def _get_current_parameter_value(self, param_id):
        ...
        parameter_values = {'07': {'winter': '60A', 'spring_autumn': '50A', 'summer': '35A'}, '10': {'winter': '60分', 'spring_autumn': '45分', 'summer': '30分'}, '62': {'winter': '60%', 'spring_autumn': '45%', 'summer': '35%'}}
        return parameter_values.get(param_id, {}).get(current_season, '未設定')

    def _get_parameter_name(self, param_id):
        """パラメータIDから日本語名を取得"""
        parameter_names = {'07': '最大充電電流', '10': '最大充電電圧充電時間', '62': 'インバータ出力切替SOC'}
    ...
    def _get_default_weather_analysis(self):
        """デフォルトの天気予報分析"""
        return {'today': {'weather': '晴れ', 'emoji': '☀️', 'solar_potential': '高い ☀️'}, 'tomorrow': {'weather': '曇り', 'emoji': '☁️', 'solar_potential': '中程度 ☁️'}, 'recommendation': '晴天を活用した充電がおすすめ'}
    ...
    def _analyze_data(self, data):
        ...
        try:
            ...
            return {'date': datetime.now().strftime('%Y年%m月%d日'), 'time': datetime.now().strftime('%H:%M'), 'battery_soc': battery_soc, 'battery_voltage': battery_voltage, 'pv_power': pv_power, 'grid_power': grid_power, 'load_power': load_power, 'daily_stats': daily_stats}
        except Exception as e:
            self.logger.error(f'データ分析エラー: {e}')
            return {'date': datetime.now().strftime('%Y年%m月%d日'), 'time': datetime.now().strftime('%H:%M'), 'battery_soc': 0, 'battery_voltage': 0, 'pv_power': 0, 'grid_power': 0, 'load_power': 0, 'daily_stats': {}}
    ...
    def _calculate_daily_stats(self, data):
        ...
        return {'total_pv_generation': round(total_pv, 2), 'total_grid_consumption': round(total_grid, 2)}
