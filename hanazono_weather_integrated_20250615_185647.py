...
def get_integrated_weather_data(self):
        ...
        try:
            ...
            if weather_result:
                # 天気データを解析してフォーマット
                formatted_weather = self.format_weather_display(weather_result)
    ...
    def format_weather_display(self, weather_data):
        """天気データ表示フォーマット"""
        try:
            # 実際の天気データを設計書準拠フォーマットに変換
            formatted = f"""🌤️ 天気予報と発電予測
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

☀️ → ☁️
今日: 実際の天気データ    明日: 実際の天気データ
気温: 実際の気温データ
発電予測: 実際のデータベース (天気API + 学習データ)"""
            
            return formatted
            
        except Exception as e:
            print(f"天気フォーマットエラー: {e}")
            return self.get_fallback_weather()
    ...
    def send_actual_email(self, subject, body):
        ...
        try:
            ...
            if email_config.GMAIL_APP_PASSWORD == "YOUR_APP_PASSWORD_HERE":
                print("⚠️ アプリパスワード未設定 - シミュレーションモード")
                return {"success": True, "mode": "simulation", "note": "app_password_required"}
