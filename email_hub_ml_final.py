...
🔧 推奨設定（6パラメーター）:
{self._format_emergency_settings_6param(alert['recommended_settings'])}

⏰ {alert['urgency']}

このメールは気象警報に基づく自動送信です。"""
            
            print("📧 緊急速報送信シミュレーション（6パラメーター）:")
            print(alert_content)
    ...
    def _format_emergency_settings_6param(self, settings: Dict[str, int]) -> str:
        """緊急設定フォーマット（6パラメーター対応）"""
        param_units = {
            "ID07": "A", "ID10": "分", "ID62": "%",
            "ID40": "時", "ID41": "時", "ID28": "V"
        }
        
        return "\n".join([
            f"ID {param}: {value}{param_units.get(param, '')}"
            for param, value in settings.items()
        ])


def main():
    """Email Hub ML Final テスト"""
    print("📧 HANAZONO Email Hub ML Integration v3.2 FINAL テスト")
    print("=" * 70)
    
    # システム初期化
    email_system = EmailHubMLFinal()
    
    print(f"📊 システム状態:")
    print(f"  ML統合: {'✅ 利用可能' if email_system.ml_available else '❌ 利用不可'}")
    print(f"  メールハブ: {'✅ 利用可能' if email_system.email_available else '❌ 利用不可'}")
    print(f"  タイプAパラメーター: 6個（ID07, ID10, ID62, ID41, ID40, ID28）")
    
    # 日次レポートテスト
    print("\n📧 日次MLレポート送信テスト（6パラメーター）:")
    ...
    print("\n✅ Email Hub ML Final テスト完了")
    print("🎯 6パラメーター対応完全統合システム完成！")
