...
class EnhancedEmailNotifier:
    ...
    def _generate_email_content(self, weather_data, recommendation, **kwargs):
        ...
        # settings.jsonのトップレベルにあるnotificationセクションからフッターを取得
    ...
    def send_daily_report(self, data, test_mode=False):
        ...
        try:
            if not self.email_config:
                self.logger.error('settings.json内に"email"キーが見つかりません。')
                return False
            ...
            # settings.jsonのトップレベルにあるnotificationセクションから件名テンプレートを取得
            subject_template = self.settings.get('notification', {}).get('email', {}).get('template', {}).get('subject', 'システムレポート')
