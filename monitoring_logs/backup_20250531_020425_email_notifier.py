...
class EmailNotifier:
    ...
    def send_daily_report(self, data):
        ...
        # vvv--- 自動挿入された環境変数解決ロジック ---vvv
        if password and isinstance(password, str) and password.startswith("${") and password.endswith("}"):
            env_var_name = password[2:-1]
            resolved_password = os.environ.get(env_var_name)
            if resolved_password:
                self.logger.info(f"環境変数 '{env_var_name}' からパスワードを解決しました。")
                password = resolved_password
            else:
                self.logger.error(f"環境変数 '{env_var_name}' が設定されていません。SMTP認証に失敗する可能性があります。")
        # ^^^--- 自動挿入された環境変数解決ロジック ---^^^
