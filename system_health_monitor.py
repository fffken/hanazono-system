...
class SystemHealthMonitor:
            ...
            if data and isinstance(data, tuple) and len(data) > 0:
                ...
                if 'parameters' in actual_data:
                    ...
                    if 'value' in soc_data and soc_data['value'] != 'N/A':
                        return {
                            'status': True,
                            'message': f"SOC正常取得: {soc_data['value']}%",
                            'soc_value': soc_data['value']
                        }
                    else:
                        return {
                            'status': False,
                            'message': 'SOC値が取得できません',
                            'auto_fix': 'communication_check'
                        }
                else:
                    return {
                        'status': False,
                        'message': 'parametersが見つかりません',
                        'auto_fix': 'data_structure_fix'
                    }
            else:
                return {
                    'status': False,
                    'message': 'データ収集が失敗しました',
                    'auto_fix': 'connection_repair'
                }
                
        except Exception as e:
            return {
                'status': False,
                'message': f'データ収集エラー: {e}',
                'auto_fix': 'module_repair'
            }
            ...
            if result == 0:
                ...
                for alt_port in alternative_ports:
                    ...
                    if alt_result == 0:
                        return {
                            'status': False,
                            'message': f'設定ポート{port}は不可、ポート{alt_port}が利用可能',
                            'auto_fix': 'port_correction',
                            'correct_port': alt_port
                        }
                
                return {
                    'status': False,
                    'message': f'通信不可: {host}:{port}',
                    'auto_fix': 'network_diagnosis'
                }
        ...
        try:
            ...
            if isinstance(data, tuple) and len(data) > 0:
                ...
                if isinstance(actual_data, dict) and 'parameters' in actual_data:
                    ...
                    return {
                        'status': False,
                        'message': 'データ構造異常: parametersキーが見つかりません',
                        'structure': structure_info,
                        'auto_fix': 'structure_adaptation'
                    }
            else:
                return {
                    'status': False,
                    'message': 'データ構造異常: 期待される形式ではありません',
                    'structure': structure_info,
                    'auto_fix': 'data_format_fix'
                }
                
        except Exception as e:
            return {
                'status': False,
                'message': f'データ構造チェックエラー: {e}',
                'auto_fix': 'collector_repair'
            }
        ...
        try:
            ...
            if 'エラー' not in battery_info and 'データなし' not in battery_info:
                return {
                    'status': True,
                    'message': 'メール機能正常',
                    'sample_output': battery_info[:100] + '...' if len(battery_info) > 100 else battery_info
                }
            else:
                return {
                    'status': False,
                    'message': f'メール機能異常: {battery_info}',
                    'auto_fix': 'email_repair'
                }
                
        except Exception as e:
            return {
                'status': False,
                'message': f'メール機能エラー: {e}',
                'auto_fix': 'email_module_repair'
            }
    ...
    def _check_config_integrity(self) -> Dict:
        ...
        try:
            ...
            if missing_sections:
                return {
                    'status': False,
                    'message': f'設定不備: {missing_sections}が見つかりません',
                    'auto_fix': 'config_completion'
                }
            ...
            return {
                'status': False,
                'message': f'設定ファイルエラー: {e}',
                'auto_fix': 'config_restoration'
            }
    ...
    def _fix_port_setting(self, correct_port: int) -> Dict:
        ...
        try:
            ...
            return {
                'status': True,
                'message': f'ポート設定を{old_port}から{correct_port}に自動修正'
            }
    ...
    def _adapt_data_structure(self) -> Dict:
        ...
        return {
            'status': True,
            'message': 'データ構造変更をログに記録。手動確認が推奨されます。'
        }
...
def run_controlled_health_check():
    ...
    if controller.should_run_full_check():
        ...
        health_report = {
            ...
            'overall_status': 'healthy',
            'checks': {
                'communication': monitor._check_communication_settings()
            }
        }
        return health_report
