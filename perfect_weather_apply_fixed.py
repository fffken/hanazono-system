#!/usr/bin/env python3
# 完璧版weather_forecast.py適用（構文エラー修正版）
import datetime
import shutil
import os
import subprocess
...
for i, day in enumerate(result.get('days', [])[:3]):
    ...
    def create_apply_record(self, backup_file, test_success):
        ...
        record_lines = [
            ...
            "```bash",
            "# 元に戻す場合",
            f"cp {backup_file if backup_file else 'weather_forecast_backup_YYYYMMDD_HHMMSS.py'} weather_forecast.py",
            "```",
            "",
        ...
        record_file = f"perfect_weather_apply_record_{self.timestamp}.md"
        with open(record_file, 'w', encoding='utf-8') as f:
            f.write(record_md)
            
        print(f"✅ 適用記録作成: {record_file}")
        return record_file
        
    def run_perfect_weather_apply(self):
        """完璧版weather_forecast.py適用実行"""
        print("🎯 完璧版weather_forecast.py適用（15分完璧化）開始")
        print("=" * 70)
        
        # 1. 元ファイルバックアップ
        backup_file = self.backup_original_weather_forecast()
        
        # 2. 完璧版適用
        apply_success = self.apply_perfect_weather_system()
        ...
        # 3. 完璧版動作テスト
        test_success = self.test_perfect_weather_system()
        
        # 4. 統合システム連携テスト
        integration_success = self.test_integration_with_perfect_weather()
        
        # 5. 適用記録作成
        record_file = self.create_apply_record(backup_file, test_success and integration_success)
        
        print(f"\n" + "=" * 70)
        print("🎉 完璧版weather_forecast.py適用完了")
        print("=" * 70)
        print(f"✅ バックアップ: {backup_file if backup_file else 'なし'}")
        print(f"✅ 完璧版適用: {'成功' if apply_success else '失敗'}")
        print(f"✅ 動作テスト: {'成功' if test_success else '失敗'}")
        print(f"✅ 統合テスト: {'成功' if integration_success else '失敗'}")
        print(f"✅ 適用記録: {record_file}")
        
        overall_success = apply_success and test_success and integration_success
        
        if overall_success:
            print(f"\n🎉 15分完璧化達成！")
            print(f"🚀 HANAZONOシステム全体が完璧版天気データを使用開始")
            print(f"⚡ 今後の全メール配信で3日分完璧気温データ配信")
            print(f"🛡️ JSONDecodeError問題完全解決")
        else:
            print(f"\n⚠️ 一部問題あり、復旧方法:")
            if backup_file:
                print(f"cp {backup_file} weather_forecast.py")
                
        return overall_success

if __name__ == "__main__":
    applier = PerfectWeatherApply()
    applier.run_perfect_weather_apply()
