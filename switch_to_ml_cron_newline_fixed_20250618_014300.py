#!/usr/bin/env python3
# ML統合版cron切り替え改行修正版（完全非破壊的）
import subprocess
import os
from datetime import datetime, timedelta

def switch_to_ml_cron_newline_fixed():
    """ML統合版cron切り替え改行修正版"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    print(f"🔄 ML統合版cron切り替え改行修正版開始 {timestamp}")
    print("=" * 70)
    ...
    try:
        ...
        if result.returncode == 0:
            backup_file = f"cron_backup_fixed_{timestamp}.txt"
    ...
    # 現在のcronから既存HANAZONOシステム行を特定・置換
    lines = current_cron.split('\n')
    new_cron_lines = []
    
    updated_count = 0
    for line in lines:
        if 'abc_integration_fixed_final' in line and updated_count < 2:
            # 既存システムをML統合版に置換
            if '7 *' in line:  # 朝のcron
    ...
    # 改行修正：末尾に確実に改行を追加
    new_cron_content = '\n'.join(new_cron_lines)
    if not new_cron_content.endswith('\n'):
        new_cron_content += '\n'
    ...
    try:
        # 新しいcronを一時ファイルに保存（改行確保）
        temp_cron_file = f"new_cron_fixed_{timestamp}.txt"
        ...
        # ファイル末尾確認
        with open(temp_cron_file, 'rb') as f:
            content = f.read()
            if content.endswith(b'\n'):
                print(f"  ✅ 改行確認: 正常")
            else:
                print(f"  ⚠️ 改行確認: 追加中")
                with open(temp_cron_file, 'ab') as f:
                    f.write(b'\n')
    ...
    if switch_success:
        ...
        print(f"  📧 ML強化メール配信開始")
        print(f"  📊 7年分データ活用開始")
        print(f"  🔄 次回メール配信: {next_morning.strftime('%H:%M') if now.hour >= 7 else next_night.strftime('%H:%M')}")
...
if __name__ == "__main__":
    switch_to_ml_cron_newline_fixed()
